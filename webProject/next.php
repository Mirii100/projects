...
// Debugging: Display submitted POST data
    ...
    if (isset($_POST['email']) && isset($_POST['password']) && !empty($_POST['email']) && !empty($_POST['password'])) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if (password_verify($password, $row["mypassword"])) {
                // Successful login, redirect to homepage with a success message
                $success = "Login successful! Welcome, " . htmlspecialchars($row['firstname']) . ".";
                header("Location: goAhead.php?success=" . urlencode($success));
